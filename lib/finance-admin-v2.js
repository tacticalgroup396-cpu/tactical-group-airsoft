...
async function requireCommander(req,res){
  ...
  const u=rows[0];if(!u){json(res,401,{error:'Faça login.'});return null}if(u.role!=='commander'){json(res,403,{error:'Acesso restrito ao comando.'});return null}return u
}
...
async function operatorDetail(req,res,url){
  const operatorId=url.searchParams.get('operator_id');if(!operatorId)return json(res,400,{error:'Operador não informado.'});
  ...
  const irregular=dues.some(x=>['pending','overdue'].includes(String(x.effective_status))&&new Date(String(x.due_date).slice(0,10)+'T23:59:59').getTime()<Date.now());
...
export default async function handler(req,res){
  try{
    await ensureSchema();const u=await requireCommander(req,res);if(!u)return;
