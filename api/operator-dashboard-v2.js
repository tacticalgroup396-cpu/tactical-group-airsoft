...
async function closeExpiredLists(){
  const games=await sql`SELECT id,title FROM games WHERE COALESCE(rsvp_closed,false)=false AND status NOT IN ('cancelado','finalizado') AND rsvp_deadline_date IS NOT NULL AND ((rsvp_deadline_date + COALESCE(rsvp_deadline_time,'23:59:59'::time)) AT TIME ZONE 'America/Sao_Paulo') <= now()`
  for(const g of games){
    const ops=await sql`SELECT id FROM operators WHERE active=true AND role IN ('operator','commander')`
    for(const o of ops)await sql`INSERT INTO game_participants(game_id,operator_id,response,present,absence_processed) VALUES(${g.id},${o.id},'pending',false,false) ON CONFLICT(game_id,operator_id) DO NOTHING`
    const pending=await sql`SELECT gp.operator_id,o.elo_level FROM game_participants gp JOIN operators o ON o.id=gp.operator_id WHERE gp.game_id=${g.id} AND gp.response='pending' AND COALESCE(gp.absence_processed,false)=false AND o.active=true`
    for(const p of pending){
      const old=Math.min(7,Math.max(1,Number(p.elo_level)||7)),next=Math.min(7,old+1)
      await sql`UPDATE operators SET elo_level=${next} WHERE id=${p.operator_id}`
      await sql`UPDATE game_participants SET absence_processed=true WHERE game_id=${g.id} AND operator_id=${p.operator_id}`
      await sql`INSERT INTO elo_history(operator_id,game_id,old_level,new_level,action,reason,changed_by) VALUES(${p.operator_id},${g.id},${old},${next},'deadline_no_response','Não respondeu à lista até o prazo',NULL)`
      await sql`INSERT INTO notifications(operator_id,type,title,body,link) VALUES(${p.operator_id},'elo_penalty','Perda de 1 Elo',${`Você não respondeu à lista de ${g.title} até o prazo e perdeu 1 Elo.`},'/operador/jogos')`
    }
    await sql`UPDATE games SET rsvp_closed=true,rsvp_closed_at=now() WHERE id=${g.id} AND COALESCE(rsvp_closed,false)=false`
  }
}
...
export default async function handler(req,res){
  try{
    ...
    const url=new URL(req.url,'http://localhost'),action=url.searchParams.get('action')||'dashboard'
    ...
    if(['dashboard','rsvp'].includes(action))await closeExpiredLists()
    ...
    if(action==='rsvp'&&req.method==='POST'){
      ...
      const game=(await sql`SELECT id,max_players,status,rsvp_closed,rsvp_deadline_date,rsvp_deadline_time FROM games WHERE id=${b.game_id} LIMIT 1`)[0];if(!game)return json(res,404,{error:'Jogo não encontrado.'});if(game.status==='cancelado')return json(res,409,{error:'Este jogo foi cancelado.'});if(game.rsvp_closed)return json(res,409,{error:'A lista deste jogo já encerrou. Não é mais possível responder.'})
      if(response==='going'){
        const settings=(await settingsRows())[0]
  ...
  }catch(e){console.error('operator-dashboard-v2',e);return json(res,500,{error:e?.message||'Erro ao carregar a área do operador.'})}
}
