...
const person=o=>({id:o.id,name:o.name,nickname:o.nickname,rank:o.rank,function:o.function||'Operador',photo_url:safePhoto(o.photo_url),elo_level:Number(o.elo_level)||7,birth_date:o.birth_date||null,age:o.age??null,visitor:!!o.visitor,team_code:o.team_code||null})
...
export default async function handler(req,res){
  try{
    ...
    const [games,roster,visitorRoster,responsible,financeRows,alertRows]=await Promise.all([
      ...
      sql`SELECT gp.game_id,gp.response,gp.loadout,o.id,o.name,o.nickname,o.rank,o.function,o.photo_url,o.elo_level,mm.team_code,false visitor FROM game_participants gp JOIN games g ON g.id=gp.game_id JOIN operators o ON o.id=gp.operator_id LEFT JOIN game_mission_members mm ON mm.game_id=gp.game_id AND mm.operator_id=gp.operator_id WHERE g.game_date>=CURRENT_DATE-interval '1 day' AND COALESCE(g.status,'')<>'cancelado' AND o.active=true ORDER BY g.game_date,o.nickname LIMIT 400`,
      sql`SELECT r.game_id,r.response,NULL::jsonb loadout,vr.id,vr.name,COALESCE(NULLIF(vr.nickname,''),vr.name) nickname,'VISITANTE'::text rank,'Visitante'::text function,'/logo.webp'::text photo_url,7::int elo_level,r.team_code,true visitor FROM visitor_game_rsvps r JOIN visitor_requests vr ON vr.id=r.visitor_request_id JOIN games g ON g.id=r.game_id WHERE g.game_date>=CURRENT_DATE-interval '1 day' AND COALESCE(g.status,'')<>'cancelado' AND r.response IN ('going','not_going') AND COALESCE(vr.status,'pending') IN ('approved','accepted') ORDER BY g.game_date,COALESCE(NULLIF(vr.nickname,''),vr.name) LIMIT 200`,
