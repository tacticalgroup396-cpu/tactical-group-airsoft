(()=>{
  ...
  async function render(){
    ...
    try{
      ...
      const o=d.operator||{};const eq=Array.isArray(d.equipment)?d.equipment:[];const gal=Array.isArray(d.gallery)?d.gallery:[];
      let minor='';
      try{const mr=await fetch('/api/public-operator?id='+encodeURIComponent(id)+'&_mobile_minor='+Date.now(),{cache:'no-store'});const md=await mr.json();if(md.minor){const g=md.guardian;minor=`<div class="profileMinorInfo"><div class="minorStatus"><span class="minorStatusIcon">⚠️</span><div><strong>Menor de idade</strong><small>Este operador possui menos de 18 anos.</small></div></div><div class="guardianStatus"><div><strong>Responsável</strong><span>${g?(g.nickname?'@'+esc(g.nickname):esc(g.name||'Não informado')):'Não informado'}</span>${g?.name?`<small>${esc(g.name)}</small>`:''}${g?.rank?`<small>${esc(g.rank)}</small>`:''}</div></div></div>`}else minor='<div class="profileMinorInfo"><div class="minorStatus"><span class="minorStatusIcon">✓</span><div><strong>Maior de idade</strong><small>Operador com 18 anos ou mais.</small></div></div></div>'}catch{minor=''}
  ...
  const scan=()=>{const id=profileId();if(id!==last)render()};document.addEventListener('click',e=>{const a=e.target?.closest?.('a[href*="/visitantes?operator="]');if(!a)return;const href=a.getAttribute('href');if(!href)return;e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();location.assign(href)},true);window.addEventListener('popstate',()=>{last='';scan()});new MutationObserver(scan).observe(document.getElementById('app')||document.body,{childList:true,subtree:true});setTimeout(scan,250);setTimeout(scan,900);setTimeout(scan,1800);
})();
