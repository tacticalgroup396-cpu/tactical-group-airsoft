...
export default async function handler(req,res){
  try{
    ...
    if(action==='commander'){
      ...
      const operators=await sql`SELECT id,name,nickname,role,rank,function,games_count,absences,elo,suspension_until,active,email,invite_expires_at,invite_used_at,is_primary_commander,last_promotion_period FROM operators ORDER BY role DESC,active DESC,nickname`
    ...
    if(action==='edit-game'&&req.method==='POST'){const u=await requireUser(req,res,'commander');if(!u)return;const b=await body(req);const min=Math.max(1,Number(b.min_players||4));const max=b.max_players?Number(b.max_players):null;if(max&&max<min)return json(res,400,{error:'O máximo não pode ser menor que o mínimo.'});await sql`UPDATE games SET min_players=${min},max_players=${max},maps_url=${String(b.maps_url||'').trim()||null},status=${b.status||'confirmado'} WHERE id=${b.game_id}`;return json(res,200,{ok:true})}
    ...
    if(action==='attendance'&&req.method==='POST'){const u=await requireUser(req,res,'commander');if(!u)return;const b=await body(req);await sql`UPDATE game_participants SET present=${!!b.present},response=CASE WHEN ${!!b.present} THEN 'attended' ELSE response END,absence_processed=false WHERE game_id=${b.game_id} AND operator_id=${b.operator_id}`;await sql`UPDATE operators SET games_count=(SELECT count(*) FROM game_participants WHERE operator_id=${b.operator_id} AND present=true) WHERE id=${b.operator_id}`;await autoPromoteEligible();return json(res,200,{ok:true})}
